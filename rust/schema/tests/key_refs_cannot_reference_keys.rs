...
fn create_schema1() -> impl Schema {
    ...
    let key_entry = builder.add_unwrap(
        "key_entry",
        TKeyRef::default(),
    );
    let key_inner = builder.add_unwrap(
        "key_inner",
        TSeq::try_new(key_entry, 0, 100).unwrap(),
    );
    ...
    // outer map
    let outer_map = builder.add_unwrap("outer_map", TMap::new(key_outer, inner_map).with_anchors(true));
