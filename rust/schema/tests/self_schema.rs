...
use liquesco_schema::schema_anchors::{SchemaAnchorsBuilder, SchemaAnchors};
...
fn test_self_schema_is_valid() {
    ...
    schema.validate(Config { no_extension : true }, &mut reader).expect("The schema itself is not valid");
}
