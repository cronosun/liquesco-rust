...
fn schema1() {
    ...
    let schema = builder.finish(TStruct::default()
        .add(Field::new(Identifier::try_from("bin").unwrap(), binary)));

    assert_valid_strict(TestData {
    ...
    // err: too short
    assert_invalid_strict(TestData {
        bin : "".as_bytes().to_vec()
    }, &schema);
    // err: too long
    assert_invalid_strict(TestData {
