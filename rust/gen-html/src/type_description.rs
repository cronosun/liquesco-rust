...
pub(crate) fn type_description(any_type: &AnyType) -> (&'static str, &'static str) {
    match any_type {
            AnyType::Struct(_) => ("structure", "A structure (aka struct) contains 0-n fields. \
            The fields do not need to be of the same type."),
            AnyType::Binary(_) => ("binary", "Arbitrary binary data."),
            AnyType::UInt(_) => ("unsigned integer", "Data of an unsigned integer (aka uint or \
            unsigned int) holds a single (positive) integer value (within a defined range)."),
            AnyType::SInt(_) => ("signed integer", "Data of signed integer (aka sint or signed \
            int) holds a single (positive or negative) integer value (within a defined range)."),
            AnyType::Ascii(_) => ("ascii", "Ascii (aka ascii text) is a sequence of characters \
            ...
            AnyType::Bool(_) => ("boolean", "Data of type boolean (aka bool) can hold the value \
            ...
            AnyType::Enum(_) => ("enumeration", "An enumeration (aka enum; tagged union; variant \
            record; discriminated union) contains 1-n variants; Each variant can (optionally) \
            have a value."),
            AnyType::Seq(_) => ("sequence", "A sequence (aka seq; list; vector; array) describes \
            a sequence of 0-n elements. Unlike struct fields, each element in a sequence has to \
            be of the same type."),
            AnyType::Float32(_) => ("float32", "A IEEE 754 32 bit float number. Do not use this \
            to transfer decimal values."),
            AnyType::Float64(_) => ("float64", "A IEEE 754 64 bit float number. Do not use this \
            to transfer decimal values."),
            AnyType::Option(_) => ("option", "Use the option type (aka maybe; optional; nullable) \
            ...
            AnyType::Unicode(_) => ("unicode", "The unicode type (aka string) can be used to \
            describe arbitrary human readable text."),
            AnyType::Uuid(_) => ("uuid", "16 byte UUID; RFC 4122."),
            AnyType::Range(_) => ("range", "A range (start - end); start/end with configurable \
            inclusion/exclusion."),
            AnyType::Map(_) => ("map", "A sequence of key-value entries. Duplicate keys are not \
            allowed. The keys can optionally be referenced to create recursive data structures."),
            AnyType::RootMap(_) => ("root_map", "A map with a root. Keys have to be unique. \
