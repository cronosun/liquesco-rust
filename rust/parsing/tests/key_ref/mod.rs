...
use liquesco_schema::structure::{TStruct, Field};
...
fn create_schema() -> DefaultSchema<'static, DefaultTypeContainer<'static>> {
    ...
    // value
    let value = builder.add_unwrap("value", TStruct::default()
        .add(Field::new(Identifier::try_from("outers").unwrap(), outers_type))
            .add(Field::new(Identifier::try_from("inners").unwrap(), inners_type))
        );

    // Inner map
    let identifier = Identifier::build_schema(&mut builder);
    let inner_map = builder.add_unwrap("inner_map", TMap::new(identifier, value).with_anchors(true));
    ...
    let outer_map = builder.add_unwrap("outer_map", TRootMap::new(root_type, outer_map_key, inner_map));
