...
impl<'a> Parser<'a> for PRootMap {
        ...
        C: Context<'c>,
    {
        ...
        if outer_seq.len()!=2 {
            return LqError::err_new(format!("The root map looks like this: \
        ...
        parse_map(context, writer, &outer_seq[1], r#type.key(),
                  r#type.value(), r#type.sorting(),
                  r#type.length(), true, false)?;
