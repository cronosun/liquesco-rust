...
fn create_identifier_schema() -> DefaultSchema<'static, DefaultTypeContainer<'static>> {
    ...
    let ascii = builder.add_unwrap(
        "ascii",
        AnyType::Ascii(TAscii::new(
            U64IneRange::try_new("", 0, 10).unwrap(),
            CodeRange::try_new(97, 123).unwrap(),
        )),
    );
    let identifier = builder.add_unwrap(
        "identifier",
        AnyType::Seq(TSeq::try_new(ascii, 1, 8).unwrap()),
    );
    let root = builder.add_unwrap(
        "root",
        AnyType::Seq(TSeq::try_new(identifier, 1, 100).unwrap()),
    );
