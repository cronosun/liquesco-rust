...
impl Common {
    ...
    pub fn fmt_u64(value : u64) -> String {
        let special_value = match value {
            U64_U8_MAX => {
                Some("Unsigned int 8 maximum")
            },
            U64_U16_MAX => {
                Some("Unsigned int 16 maximum")
            },
            U64_U32_MAX => {
                Some("Unsigned int 32 maximum")
            },
            std::u64::MAX => {
                Some("Unsigned int 64 maximum")
            },
            _ => None
        };
    ...
    pub fn fmt_i64(value : i64) -> String {
        ...
        let special_value = match value {
            I64_U8_MAX => {
                Some("Signed int 8 maximum")
            },
            I64_U16_MAX => {
                Some("Signed int 16 maximum")
            },
            I64_U32_MAX => {
                Some("Signed int 32 maximum")
            },
            std::i64::MAX => {
                Some("Signed int 64 maximum")
            },
            I64_U8_MIN => {
                Some("Signed int 8 minimum")
            },
            I64_U16_MIN => {
                Some("Signed int 16 minimum")
            },
            I64_U32_MIN => {
                Some("Signed int 32 minimum")
            },
            std::i64::MIN => {
                Some("Signed int 64 minimum")
            },
            _ => None
        };
        if let Some(special_value) = special_value {
    ...
    pub fn fmt_i128(value : &i128) -> String {
        if let Ok(i64_value) = i64::try_from(*value) {
            ...
            match value {
                ...
                &std::i128::MIN => {
                    format!("Unsigned int 128 minimum")
                },
                _ => {
                    ToFormattedString::to_formatted_string(value, &Locale::en)
                }
    ...
    pub fn fmt_f64(value : &f64) -> String {
        Self::fmt_float(value.classify(), value.is_sign_negative(),
                        value, &std::f64::MIN, &std::f64::MAX)
    }

    pub fn fmt_f32(value : &f32) -> String {
        Self::fmt_float(value.classify(), value.is_sign_negative(),
                        value, &std::f32::MIN, &std::f32::MAX)
    }

    pub fn fmt_decimal(value : &Decimal) -> String {
        match value {
            &Decimal::MAX => {
                format!("Decimal maximum value")
            },
            &Decimal::MIN => {
                format!("Decimal minimum value")
            },
            _ => {
                format!("{}", value)
            }
    ...
    fn fmt_float<T>(
        ...
        value : &T,
        min : &T,
        max : &T) -> String where T : Display + PartialEq {
