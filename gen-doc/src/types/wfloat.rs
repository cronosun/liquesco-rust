...
use liquesco_schema::types::float::{TFloat, TFloat32, TFloat64};
use std::fmt::Debug;
...
fn float_range<F>(rows: &mut Vec<Row>, range: &Range<F>, start: String, end: String) {
    rows.push(Row::association_with_text("Min value", start));
    rows.push(Row::association_with_text(
        "Min included",
        Common::fmt_bool_yes_no(range.start_included()),
    ));
    rows.push(Row::association_with_text("Max value", end));
    rows.push(Row::association_with_text(
        "Max included",
